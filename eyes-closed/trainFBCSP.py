#!/usr/bin/env python3
"""
train_two_stage_ICA_FBCSP_19ch.py
...
epoch rejection, as per the provided research.
"""
...
# (Note: MNE standardizes T3/T4/T5/T6 to T7/T8/P7/P8)
...
random_state = 42
# This threshold is now applied to *cleaned* data, so it's fine.
reject_criteria = dict(eeg=150e-6) 

# --- NEW FILENAMES for the 19-channel models ---
model1_fname = R"D:\Prsnl\ML\py codes\BCI_Project\src\bci_stage1_rest_intent_model_19ch_ICA.joblib"
model2_fname = R"D:\Prsnl\ML\py codes\BCI_Project\src\bci_stage2_left_right_model_19ch_ICA.joblib"
...
# --------------------------
print("="*60)
print(" STAGE 1: TRAINING 'REST' vs. 'INTENT' (19-Ch, FBCSP+ICA)")
print("="*60)
...
for subj in subjects:
    try:
        ...
        for raw, task_type in [(raw_rest, 'rest'), (raw_intent, 'intent')]:
            ...
            # --- LOGIC FIX: Pick *only* the 19 model channels ---
            ...
            if len(picks_19) < 19:
                ...
                continue # Skip this subject's data
            raw.pick_channels(picks_19, ordered=False)
            
            # EOG channels are now 'Fp1' and 'Fp2', which are in our list
            ...
            # --- Notch filter *before* ICA (as per research) ---
            ...
            # This reject_criteria is now applied to CLEANED data
            # This fixes the "zero-size array" crash
...
# --------------------------
print("\n" + "="*60)
print(" STAGE 2: TRAINING 'LEFT' vs. 'RIGHT' (19-Ch, FBCSP+ICA)")
print("="*60)
