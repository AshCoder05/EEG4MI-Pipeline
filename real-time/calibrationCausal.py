#!/usr/bin/env python3
"""
calibrate_s8_FINETUNE_ICA_CAUSAL.py

FINAL SCRIPT:
- Includes user's (s8) data loading.
- Includes metadata generation.
- Includes identical ICA preprocessing step.
- *** FIX: Matches the CAUSAL (IIR) settings of the new training script. ***
- *** FIX: Disables epoch rejection to handle noisy 's8' data. ***
"""
...
# --- 2. Paths to the *NEW CAUSAL* models you are training ---
...
CALIBRATED_MODEL_S1_FNAME = R"D:\Prsnl\ML\py codes\BCI_Project\src\s8_causal_stage1_model.joblib"
CALIBRATED_MODEL_S2_FNAME = R"D:\Prsnl\ML\py codes\BCI_Project\src\s8_causal_stage2_model.joblib"
...
feature_tmin, feature_tmax = 0.0, 2.0
epoch_duration_for_rest = 3.5 # tmax - tmin

# --- *** NEW: Match training script *** ---
causal_filter = True
notch_freqs = [50.0]
global_sfreq = 160.0
...
# --- *** FIX: Disable rejection criteria *** ---
...
# -------------------------------------------------------------
# --- UPDATED DATA LOADING FUNCTION (CAUSAL + ICA) ---
# -------------------------------------------------------------
...
if __name__ == "__main__":
    try:
        ...
        print("="*60)
        print(" STAGE 1: CALIBRATING 'REST' vs. 'INTENT' (with ICA, CAUSAL)")
        print("="*60)
        ...
        # We use reject_criteria=None to avoid dropping all epochs
        ...
        print("\n" + "="*60)
        print(" STAGE 2: CALIBRATING 'LEFT' vs. 'RIGHT' (with ICA, CAUSAL)")
        print("="*60)
